...
class OverlayService : Service() {
    ...
    private var digterSliderBox: View? = null
    private var btnBH: TextView? = null
    private var btnDG: TextView? = null
    ...
    private fun buildPanel(): View {
        ...
        btnDG = TextView(this).apply {
            text = "DIGTER"; gravity = Gravity.CENTER
            setTextColor(Color.parseColor("#5C647E"))
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 11f)
            letterSpacing = 0.08f
        ...
        btnBH!!.setOnClickListener {
            ...
            selectToggle(btnBH!!, btnDG!!)
            revealView(bigheadSliderBox!!); hideView(digterSliderBox!!); hideCircle()
        }
        btnDG!!.setOnClickListener {
            if (currentMode == Mode.DIGTER) return@setOnClickListener
            currentMode = Mode.DIGTER
            selectToggle(btnDG!!, btnBH!!)
            revealView(digterSliderBox!!); hideView(bigheadSliderBox!!); showCircle()
        }

        toggleRow.addView(btnBH)
        toggleRow.addView(btnDG)

        card.addView(dragRow)
        card.addView(toggleRow)

        // ── BIGHEAD slider box ─────────────────────────────────────────────────
        bigheadSliderBox = buildSliderBox("HEAD SIZE", isDigter = false)
        // ── DIGTER slider box ──────────────────────────────────────────────────
        digterSliderBox = buildSliderBox("CIRCLE SIZE", isDigter = true)
        digterSliderBox!!.visibility = View.GONE
        digterSliderBox!!.alpha = 0f
        ...
        root.addView(digterSliderBox, LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT,
            LinearLayout.LayoutParams.WRAP_CONTENT).also { it.topMargin = px(6f) })
    ...
    private fun buildSliderBox(label: String, isDigter: Boolean): View {
        ...
        val lbl = TextView(this).apply {
            text = label
            setTextColor(if (isDigter) Color.parseColor("#00E5FF") else Color.parseColor("#00E5FF"))
        ...
        val seek = SeekBar(this).apply {
            max = 100
            progress = if (isDigter) 40 else 50
            progressTintList = android.content.res.ColorStateList.valueOf(
                if (isDigter) Color.parseColor("#00E5FF") else Color.parseColor("#7C4DFF"))
